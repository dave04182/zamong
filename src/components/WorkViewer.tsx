          ...
          >
            ...
            <div style={{
              ...
              {isTextGenre(work.genre) && work.content_text && (
                <div
                  data-text-panel="true"
                  style={{ position: 'absolute', inset: 0, background: darkMode ? '#1E1B18' : '#FDFAF4', overflowY: 'auto', padding: '60px' }}
                >
                  <button
                    ...
                    style={{
                      ...
                      fontSize: '11px', color: darkMode ? 'rgba(255,255,255,0.5)' : '#AFA79F',
                      cursor: 'pointer', letterSpacing: '1px', marginBottom: '16px',
                    }}
                  >
                    {darkMode ? '☀ 밝게' : '🌙 어둡게'}
                  </button>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {work.tags && work.tags.length > 0 && (
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '20px' }}>
                    {work.tags.map(tag => (
                      <span key={tag} style={{ display: 'inline-block', background: '#EFE6D5', color: '#8A6F4A', borderRadius: '20px', padding: '3px 10px', fontSize: '11px' }}>
                        #{tag}
                      </span>
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button
                    ...
                    style={{
                      ...
                      background: likedIds.includes(work.id) ? '#FDF0E4' : 'none',
                      border: `0.5px solid ${likedIds.includes(work.id) ? '#EDD9BC' : 'rgba(110,90,60,0.22)'}`,
                      borderRadius: '8px', padding: '8px 16px', fontSize: '12px',
                      color: likedIds.includes(work.id) ? '#C17B3F' : '#78706A',
                  ...
                  <button
                    ...
                    style={{
                      display: 'flex', alignItems: 'center', gap: '5px',
                      background: bookmarkedIds.includes(work.id) ? '#FDF0E4' : 'none',
                      border: `0.5px solid ${bookmarkedIds.includes(work.id) ? '#EDD9BC' : 'rgba(110,90,60,0.22)'}`,
                      borderRadius: '8px', padding: '8px 16px', fontSize: '12px',
                      color: bookmarkedIds.includes(work.id) ? '#C17B3F' : '#78706A',
                      cursor: 'pointer', fontFamily: 'inherit',
                    }}
                  >
              ...
              {idx === currentIndex && (
                <div style={{
                  position: 'absolute', inset: 0,
                  background: '#FDFAF4',
                  transform: showReviews ? 'translateY(0)' : 'translateY(100%)',
                  transition: 'transform 0.45s cubic-bezier(0.25, 0.46, 0.45, 0.94)',
                  overflowY: 'auto',
                  zIndex: 10,
                }}>
                  <div style={{ position: 'sticky', top: 0, background: '#FDFAF4', padding: '16px 24px 12px', borderBottom: '0.5px solid rgba(110,90,60,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '13px', fontWeight: 500, color: '#26211C' }}>감상문</span>
                    <button
                      onClick={() => setShowReviews(false)}
                      style={{ fontSize: '12px', color: '#AFA79F', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
                    >
                      ✕ 닫기
                    </button>
                  </div>
